 ...
 */
async function init() {
  var user = await User.findOne({ userId: "Harsha" });
  ...
  try {
    user = await User.create({
      name: "Harsha Vardhan",
      userId: "Harsha", // It should be atleat 16, else will throw error
      email: "harshavardhan81795@gmail.com", // If we don't pass this, it will throw the error
      userType: "ADMIN",
      password: bcrypt.hashSync("Harsha", 8), //this field should be hidden from the end user
    });
    console.log(user);
  } catch (e) {
